...
//				console.log('#demographics', d.fetch())
//				console.log('#patients',p.fetch())
//				var r = _.chain(d).union(p).sortBy("Patient_ID").uniq(true, function(item){ return item.Patient_ID; }).value();
				var r = _.uniq(_.union(d, p), false, function(item, key){ return item.Patient_ID; });
				console.log('#union',r)
   				console.log("Demographics count", d.count(),"Patient Enrollment count", p.count());
                return r;
            }else {
