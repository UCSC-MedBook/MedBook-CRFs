...
CRFs = [
    ...
    // Obsolete: 'Patient_Enrollment_form',
...
ingestOncore = function () {
	...
	p_sample_list.forEach(function(sample) {	
		var sample_id = sample['Sample_ID']
		if (sample_id != 'DTB-MISSING') {
			if (!samples[sample_id]) {
				samples[sample_id] = {}
			}
			if (sample.Drug_Name && sample.Drug_name != 'undefined') {
				if (!samples[sample_id]['treatment_for_mcrpc_prior_to_biopsy']) {
					samples[sample_id]['treatment_for_mcrpc_prior_to_biopsy'] = []
					console.log('empty drugs',sample_id)
				}
				//console.log('before drug',sample_id, sample.Drug_Name, samples[sample_id]['treatment_for_mcrpc_prior_to_biopsy'])
				samples[sample_id]['treatment_for_mcrpc_prior_to_biopsy'].push(sample.Drug_Name)
				//console.log('after drug',sample_id, sample.Drug_Name, samples[sample_id]['treatment_for_mcrpc_prior_to_biopsy'])
			}
			if (sample.Reason_for_Stopping_Treatment) {
				samples[sample_id]['Reason_for_Stopping_Treatment'] = sample.Reason_for_Stopping_Treatment
			}
			//samples.data = data
			//console.log('prior treatment',sample_id, samples[sample_id])
		}
	})
	console.log('prior finished',samples)
	...
	sample_list.forEach(function(sample) {	
	    ...
	    if (sample['Site']) {
			...
			data['Study_Terminated'] = 'True'
			//console.log("no site - biopsy skipped",sample)
		}
	})
