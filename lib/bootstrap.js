...
map_Drug_Name = function(anObj) {
    ...
    // 1. Delete prednisone
    var drugs = anObj.Drug_Name.split("; ").filter(function(d) {
...
ingestOncore = function () {
	...
	//var blue = new Mongo.Collection("Clinical_Info");
	...
	sample_list.forEach(function(sample) {	
	    ...
	    if (sample['Site']) {
			data.biopsy_site = sample['Site']
			data['Patient_ID'] = sample['Patient_ID']
			console.log("clinical info biopsy", data,'input',sample)
			map_biopsy_site(data);
