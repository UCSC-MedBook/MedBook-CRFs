  ...
  ingestClinical = function () {
  		...
  		if (drug) {
  			...
  			prior[sample_id][drug] = 'Resistant'
                ...
                ret = clinical_info.update(
                            {'Sample_ID':sample_id},
                            { $push: { prior_txs: summarizeTreatment("SU2C_Prior_TX_V3", treatment) }});
                console.log("clinical_info ret D",  ret);
  	})
  		...
  		if (prior[sample_id]) {
  			...
  			if (samples[sample_id] && samples[sample_id] != undefined) {
  				try {
  						...
  						{$set:samples[sample_id]},
  						{upsert:true} );
                                    console.log("clinical_info ret C",  ret);
  				}
  		        catch (ex) {
  	...
  	treatment_list.forEach(function(treatment) {
  		...
  		var ret = clinical_info.update(
  			...
  			{$set:data},
  			{upsert:true})
                console.log("clinical_info ret A",  ret);
                ret = clinical_info.update( 
                            {'Sample_ID':sample_id},
                            { $push: { subsequent_txs: summarizeTreatment("SU2C_Subsequent_Treatment_V1", treatment) }});  // TCG 8/2/2015
                console.log("clinical_info ret B",  ret);
  	})
